...
```{r}
pheno_chr11 <- pheno %>% 
  full_join(genotypes_chr11, by = "IID")

pheno_chr20 <- pheno %>% 
  full_join(genotypes_chr20, by = "IID")
```
...
Since p value is above 0.05, the result is not significant. The null hypothesis cannot be rejected and this particular genotype does not have a major impact on height.
...
```{r}
manhattan_plot <- height_sumstats_chr20 %>% 
  mutate(logvalues = log10(P))

manhattan_plot %>% 
  ggplot(aes(x = POS, y = -logvalues)) +
  geom_point() +
...
![](images/clipboard-2430492957.png)
...
manhattan_plot_11 <- height_sumstats_chr11 %>% 
  mutate(logvalues = log10(P))
...
![](images/clipboard-4265530057.png)
